...
def run_cv_training(seq_length, hidden_dim, epochs, use_early_stopping=False, save_best_model=False):
    """
    Executes the 5-fold Subject-Wise CV for a specific LSTM configuration.
    ...
    if save_best_model and best_model_weights is not None:
        torch.save(best_model_weights, "best_lstm_mega_model.pth")
        
        # Save Mega Training TXT Report
        report = [
            "="*70,
            f"LSTM MEGA TRAINING REPORT (Time Steps: {seq_length}, Hidden: {hidden_dim})",
            "="*70,
        ...
        report.extend([
            ...
            "-"*70,
            f"Best Model Saved: best_lstm_mega_model.pth (from Fold {best_fold_identifier})"
        ])
        with open("lstm_mega_training_report.txt", "w") as f:
            f.write("\n".join(report))
...
# ==========================================
print("\n" + "="*70)
print(f"PHASE 1: LSTM GRID SEARCH (Epochs: {GRID_EPOCHS})")
print("="*70)

best_grid_f2 = -np.inf
best_grid_config = {}

with open("lstm_grid_search_results.txt", "w") as f:
    f.write("LSTM GRID SEARCH RESULTS\n" + "-"*50 + "\n")
...
print("\nProcess fully completed! Check 'lstm_grid_search_results.txt' and 'lstm_mega_training_report.txt'.")