...
SEQ_LENGTH = 7
HIDDEN_DIM = 32
TARGET_FOLD = 3 # Fold 3 is the one from which the best_model was saved
...
def main():
    print("1. Loading data and isolating Validation Fold (Fold 3)...")
    ...
    # Sliding Windows only on Test Set of Fold 3
    ...
    print("2. Loading the saved LSTM model (Best Model)...")
    ...
    # Update this path if the .pth file is in a different folder
    model.load_state_dict(torch.load("best_lstm_mega_model.pth", map_location=DEVICE))
    model.eval()
    ...
    plt.plot(mean_predicted_value_uncal, fraction_of_positives_uncal, "s-", label=f"LSTM (Uncalibrated) - ECE: {ece_uncal:.4f}")
    plt.plot(mean_predicted_value_cal, fraction_of_positives_cal, "o-", label=f"LSTM (Calibrated) - ECE: {ece_cal:.4f}")
    plt.ylabel("True Injury Frequency")
    plt.xlabel("Predicted Probability")
    plt.title("Reliability Diagram (Calibration Curve) - Class 2 (Injury)")
    plt.legend(loc="best")
    plt.grid(True)
    
    plot_path = os.path.join(OUTPUT_DIR, "calibration_curve_class2.png")
    plt.savefig(plot_path)
    print(f"   Plot saved to: {plot_path}")
    
    # Save Numerical Results
    txt_path = os.path.join(OUTPUT_DIR, "calibration_numerical_results.txt")
    with open(txt_path, "w") as f:
        f.write("=== LSTM CALIBRATION RESULTS ===\n")
        f.write(f"Model: best_lstm_mega_model.pth (Fold 3)\n")
        ...
        f.write("\nThis file does not alter the network weights; it only converts raw logits ")
        f.write("into statistically valid probabilities for the clinical interface.\n")
