...
# Import your modules
from MultimodalSystemSportsInjury.GRU.model import InjuryPredictionGRU
...
SEQ_LENGTH = 7
HIDDEN_DIM = 32
# TARGET_FOLD is set to 1 based on the gru_mega_training_report.txt
TARGET_FOLD = 1 
...
def main():
    ...
    # We look exactly for the TARGET_FOLD to prevent data leakage
    ...
    print("2. Loading the saved GRU model (Best Model)...")
    ...
    # Update the path if best_gru_mega_model.pth is in a different folder
    model.load_state_dict(torch.load("best_gru_mega_model.pth", map_location=DEVICE))
    model.eval()
    ...
    plt.plot(mean_predicted_value_uncal, fraction_of_positives_uncal, "s-", label=f"GRU (Uncalibrated) - ECE: {ece_uncal:.4f}")
    plt.plot(mean_predicted_value_cal, fraction_of_positives_cal, "o-", label=f"GRU (Calibrated) - ECE: {ece_cal:.4f}")
    plt.ylabel("True Injury Frequency")
    plt.xlabel("Predicted Probability")
    plt.title("Reliability Diagram (Calibration Curve) - GRU Class 2")
    plt.legend(loc="best")
    plt.grid(True)
    
    plot_path = os.path.join(OUTPUT_DIR, "gru_calibration_curve_class2.png")
    plt.savefig(plot_path)
    print(f"   Plot saved to: {plot_path}")
    
    # Save Numerical Results
    txt_path = os.path.join(OUTPUT_DIR, "gru_calibration_numerical_results.txt")
    with open(txt_path, "w") as f:
        f.write("=== GRU CALIBRATION RESULTS ===\n")
        f.write(f"Model: best_gru_mega_model.pth (Fold {TARGET_FOLD})\n")
